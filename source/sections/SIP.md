...
Le SEDA dans ses versions 2.1. et suivantes représente ce groupe d’objets de deux manières différentes :
- création du groupe d’objets (DataObjectGroup) en utilisant la balise *DataObjectGroup* et création, dans cette balise *DataObjectGroup*, des objets qui le constituent. Chaque objet doit déclarer la version ou l’usage auquel il correspond via la balise *DataObjectVersion* (méthode propre au SEDA dans ses versions 2.1. et suivantes, et à privilégier) ;
...
|Version du SEDA|SEDA 2.1, SEDA 2.2 ou SEDA 2.3[^3].|Obligatoire|
...
|Identifiant pérenne|Au terme de la version 7.1, la solution logicielle Vitam met à disposition un bloc <PersistentIdentifier> qui peut être utilisé pour signaler un identifiant pérenne.<br>
À noter que ce bloc ne doit être utilisé pour identifier un objet que dans la version 2.3. du SEDA.|Obligatoire|
